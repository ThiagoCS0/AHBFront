...
export default function Gerenciar({ dados_offline, apis }) {
  ...
  useEffect(() => {
    ...
    mudanca_sessionstorage(); def_carregando(false);
    return () => {
  ...
  const mudanca_sessionstorage = () => {
    ...
    try {
      if (pagina_atual) {
        ...
        if (pagina_atual.includes("{") && pagina_atual.includes("}")) { possivel_objeto = JSON.parse(pagina_atual); }
        if (possivel_objeto !== null && typeof possivel_objeto === "object") {
          def_aba(possivel_objeto.aba); def_pagina(possivel_objeto.pag); pagina_atual = possivel_objeto.pag;
        }
        else { def_pagina(pagina_atual); }
      }
  ...
  const aguardar_elemento = (pagina_atual, saida) => {
    ...
    const verificar = () => {
      ...
      if (elemento) { saida(elemento); }
      else if (tentativas < 10) { tentativas++; setTimeout(verificar, 100); }
    };
    verificar();
  };
  ...
  useEffect(() => {
    if (validar_token()) {
      switch (pagina) {
        case "MinhasAPIs": def_nome_pagina("Minhas APIs"); break;
        case "Perfil": def_nome_pagina("Perfil"); break;
        default: break;
      }
    }

    switch (pagina) {
      case "Sobre": def_nome_pagina("AHB - API HUB BRASIL"); break;
      case "Termos": def_nome_pagina("Termos de Uso"); break;
      case "Parceiros": def_nome_pagina("Parceiros"); break;
      case "Doacao": def_nome_pagina("Doação"); break;
      case "Ranks": def_nome_pagina("Ranks"); break;
      default: break;
    }
  }, [pagina]);
  ...
  const componentes = () => {
    if (validar_token()) {
      switch (pagina) {
        ...
        case "MinhasAPIs": return;
        default: break;
      }
    }

    switch (pagina) {
      case "Ranks": return <Ranks dados_offline={dados_offline} aba={aba || "ranks_populares"} apis={apis} />;
    ...
    <div id="gerenciar">
      <div id="gerenciar_menu">
        {!dados_offline && validar_token() && (
          <>
            <Link id="Perfil" onClick={() => alterar_componente("Perfil")}>
              <img src={icon_usuario} className="gerenciar_icones" alt="Perfil" /> <p className="gerenciar_texto">Perfil</p>
            </Link>
            <Link id="MinhasAPIs" onClick={() => alterar_componente("MinhasAPIs")}>
              <img src={icon_minhas_apis} className="gerenciar_icones" alt="Perfil" /> <p className="gerenciar_texto">Minhas APIs</p>
            </Link>
          </>
        )}
        <Link id="Parceiros" onClick={() => alterar_componente(null, JSON.stringify({ pag: "Parceiros", aba: "parceiros_instituicoes" }))} className="desativado" style={{ pointerEvents: 'none' }}>
          <img src={icon_parceria} className="gerenciar_icones" alt="Parceiros" /> <p className="gerenciar_texto">Parceiros</p>
        </Link>
        <Link id="Doacao" onClick={() => alterar_componente("Doacao")} className="desativado" style={{ pointerEvents: 'none' }}>
          <img src={icon_doacao} className="gerenciar_icones" alt="Doação" /> <p className="gerenciar_texto">Doação</p>
        </Link>
        <Link id="Ranks" onClick={() => alterar_componente(null, JSON.stringify({ pag: "Ranks", aba: "ranks_populares" }))}>
          <img src={icon_ranks} className="gerenciar_icones" alt="Ranks" /> <p className="gerenciar_texto">Ranks</p>
        </Link>
        <Link id="Sobre" onClick={() => alterar_componente("Sobre")}>
          <img src={icon_sobre} className="gerenciar_icones" alt="Sobre" /> <p className="gerenciar_texto">Sobre</p>
        </Link>
        <Link id="Termos" onClick={() => alterar_componente("Termos")}>
          <img src={icon_termos} className="gerenciar_icones" alt="Termos" /> <p className="gerenciar_texto">Termos</p>
        </Link>
      </div>
      <div id="gerenciar_conteudo">
        ...
        <div className="gerenciar_pagina">
          {pagina === "MinhasAPIs" ?
            <MinhasAPIs
              dados_offline={dados_offline} editar_api={editar_api} exibir_modal_editar={exibir_modal_editar}
              def_editar_api={def_editar_api} def_exibir_modal_editar={def_exibir_modal_editar} />
            :
            componentes()
          }
        </div>
